...
class SabankPipeline:

    # Database setup
    conn = sqlite3.connect('sabank.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `sabank`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM sabank WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO sabank (date, title, link, content)"
